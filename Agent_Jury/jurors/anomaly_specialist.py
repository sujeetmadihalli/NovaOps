...
# These cover categories the other 3 jurors are NOT specialised in.
# Each entry: (category_name, [regex patterns], recommended_action)
...
class AnomalySpecialistJuror(BaseJuror):
    ...
    def _pattern_scan(self, incident_context: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Layer 2: Regex scan across all raw context strings.
        Returns list of matched categories with their evidence snippets.
        """
        # Flatten all context into one searchable string
    ...
    def _rag_confidence_signal(self, incident_context: Dict[str, Any]) -> str:
        """
        Layer 1: Use the runbook content as a RAG confidence signal.
        If no runbook matched, that's a strong unknown indicator.
        """
        runbook = incident_context.get("runbook", "")
        if "No strictly relevant runbook found" in runbook or "No runbook" in runbook or not runbook.strip():
...
"""
    ...
    def deliberate(self, incident_context: Dict[str, Any]) -> Dict[str, Any]:
        """
        Override to pre-run pattern scan and boost confidence when strong pattern matches exist.
        In mock mode, still runs the pattern scan against the mock context for realism.
        """
        pattern_hits = self._pattern_scan(incident_context)
        verdict = super().deliberate(incident_context)  # handles mock mode via base class

        # If pattern scan found strong signals but LLM was uncertain, boost confidence
        if pattern_hits and verdict.get("confidence", 0.0) < 0.5:
            logger.info(f"[{self.name}] Pattern scan found {len(pattern_hits)} category matches — boosting confidence signal")
            verdict["confidence"] = max(verdict["confidence"], 0.6)
            verdict["detected_categories"] = [h["category"] for h in pattern_hits]

        # Always attach detected categories to the verdict
    ...
    def _mock_deliberate(self) -> Dict[str, Any]:
        return {
            "juror": self.name,
            "verdict": "No external or anomalous factors detected. Incident appears to be within known categories.",
            "confidence": 0.1,
            "recommended_action": "noop_require_human",
            "reasoning": "Pattern scan found no external API, network, database, or security signatures. RAG matched a known runbook. Deferring to other jurors.",
            "detected_categories": []
        }
